...
public class CSC205_LogicTests_Program2 {
    ...
    private static void testSavingsAccountLogic() throws Exception {
        ...
        if (assertEqualText(System.out, expectedInfo.replace(" ", "").replace("\t", ""), actualInfo.replace(" ", "").replace("\t", ""),
            ...
            fail++; 
            System.out.println("NOTE: in the above text, spaces have been replaced with ' '");
    ...
    private static void testCheckingAccountLogic() throws Exception {
     // Default constructor initializes accountNumber to "0000-0000-0000-0000"
        ...
        String expectedInfo = "Account type  : Checking\n" +
                              ...
                              "Overdraft fee : $20.00";
        String actualInfo = acc.getAccountInfo().trim();
        if (assertEqualText(System.out, expectedInfo.replace(" ", "").replace("\t", ""), actualInfo.replace(" ", "").replace("\t", ""),"with accountNumber \"111-222-333-444\", balance 1261, and interestRate  0.123, getAccountInfo returns correct string"))
            pass++;
        else {
            fail++; 
    ...
    private static void testCreditcardAccountLogic() throws Exception {
     ...
     // Default constructor initializes creditLimit to 0
        ...
        expectedBalance = -1000;
        actualBalance = (int) getBaseFieldValue(acc, "balance");
        if (assertEqual(System.out, expectedBalance, actualBalance,"from current balance of -1050, and a credit(50), getBalance returns -1000"))
            pass++;
        else
            fail++;
        ...
        expectedBalance = -1123;
        actualBalance = (int) getBaseFieldValue(acc, "balance");
        if (assertEqual(System.out, expectedBalance, actualBalance,"with current balance of -1000, and interestRate of 0.123, and applyInterest(), getBalance returns -1123"))
            pass++;
        else
            fail++;
        ...
        expectedBalance = -1261;
        actualBalance = (int) getBaseFieldValue(acc, "balance");
        if (assertEqual(System.out, expectedBalance, actualBalance,"with current balance of -1123, and interestRate of 0.123, and applyInterest(), getBalance returns -1261"))
            pass++;
        else
            fail++;
